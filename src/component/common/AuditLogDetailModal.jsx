  ...
  return (
    <div className="space-y-1">
      ...
      {rows.map((row, idx) => (
        <div
          key={idx}
          className="grid grid-cols-12 gap-4 px-4 py-2.5 hover:bg-slate-50 transition-colors border-b border-slate-100 items-center"
        >
          <div className="col-span-4">
            <span className=" text-[11px] text-slate-500 ">{row.property}</span>
          </div>
          {opLabel !== "Create" && (
            <div className="col-span-4">
...
export function AuditLogDetailsContent({
  ...
  try {
    if (
      ...
      const po =
        typeof item.oldValues === "string"
          ? JSON.parse(item.oldValues)
          : item.oldValues;
  ...
  const affectedArr = Array.isArray(item.affectedColumnsArr)
    ? item.affectedColumnsArr
    : [];
  ...
  const hasOldVal =
    (parsedOld && Object.keys(parsedOld).length > 0) ||
  ...
  const formatValue = (val) => {
    ...
    if (typeof val === "object" && val !== null) {
      ...
      if (val.Name !== undefined && val.Name !== null) {
        const nameStr = Array.isArray(val.Name)
          ? val.Name.join("")
          : String(val.Name);
  ...
  const columns = [
    {
      ...
      flex: 1,
      renderCell: (params) => (
        <span className=" text-[11px] text-slate-500 ">{params.value}</span>
      ),
    },
  ];
  ...
  if (opLabel !== "Create") {
    columns.push({
      ...
      flex: 1.5,
      renderCell: (params) => (
        <span className="text-[12px] text-slate-400 line-through decoration-slate-300">
          {params.value}
        </span>
      ),
    });
  ...
  columns.push({
    ...
    flex: 1.5,
    renderCell: (params) => (
      <span className=" text-[12px] text-emerald-600 dark:text-emerald-400 bg-emerald-50/50 dark:bg-emerald-500/10 px-2 py-0.5 rounded-md border border-emerald-100/50">
        {params.value}
      </span>
    ),
  });
    ...
    >
      ...
      {!hideHeader && (
        ...
        >
          <Typography
            variant="body2"
            sx={{ fontWeight: 900, fontSize: "0.7rem", color: "primary.light" }}
          >
            —
          </Typography>
          <Typography
      ...
      {!isCollapsible && (
        ...
        >
          <button
            onClick={onClose}
            className="w-full py-2.5 bg-slate-900 text-white rounded-xl text-[10px] tracking-[0.2em] hover:bg-black transition-all shadow-xl shadow-slate-200"
          >
            Close Inspection
          </button>
        </Box>
