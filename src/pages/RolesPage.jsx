    ...
    const columns = useMemo(() => [
        {
            ...
            render: (val, row) => (
                <div className="flex items-center gap-2.5">
                    <span className="text-sm font-bold text-slate-800 dark:text-white capitalize truncate">{val}</span>
                    {row.isDefault && (
                        <span className="text-blue-500 text-[10px] font-bold capitalize tracking-wide">Default</span>
                    )}
                    {row.isPublic && (
                        <span className="text-[#4188a1] text-[10px] font-bold capitalize tracking-wide">Public</span>
                    )}
                </div>
            )
        }
        ...
        return (
            <div className="fixed inset-0 z-100 flex items-center justify-center p-4">
                ...
                <div className="relative bg-white dark:bg-[#1e2436] rounded-[24px] p-6 w-full max-w-[360px] shadow-2xl shadow-blue-500/10 border border-slate-100 dark:border-white/10 transition-all duration-300">
                    <h2 className="text-base font-black dark:text-white mb-4 uppercase tracking-tight text-slate-800">
                        {item ? 'Edit Role' : 'Create Role'}
                    </h2>
                    <div className="space-y-3">
                        <div>
                            <label className="block text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1.5 ml-1">Role Name</label>
                            <input
                                id="role-name-input"
                                type="text"
                                defaultValue={item?.name || ''}
                                className="w-full px-4 py-2.5 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl outline-none focus:ring-2 focus:ring-blue-500/20 dark:text-white font-bold text-sm transition-all duration-200"
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <label className="flex flex-col gap-1 px-3 py-2 border-2 border-slate-100 dark:border-white/10 rounded-xl hover:bg-white hover:shadow-[0_4px_12px_rgba(0,0,0,0.05)] hover:-translate-y-0.5 dark:hover:bg-white/5 cursor-pointer transition-all duration-200 group">
                                <div className="flex items-center justify-between">
                                    <span className="text-[11px] font-bold text-slate-700 dark:text-slate-300 uppercase tracking-tight group-hover:text-blue-600 transition-colors">Default</span>
                                    <input 
                                        type="checkbox" 
                                ...
                                <span className="text-[8px] text-slate-400 font-bold uppercase tracking-wide">Assign new users</span>
                            </label>
                            <label className="flex flex-col gap-1 px-3 py-2 border-2 border-slate-100 dark:border-white/10 rounded-xl hover:bg-white hover:shadow-[0_4px_12px_rgba(0,0,0,0.05)] hover:-translate-y-0.5 dark:hover:bg-white/5 cursor-pointer transition-all duration-200 group">
                                <div className="flex items-center justify-between">
                                    <span className="text-[11px] font-bold text-slate-700 dark:text-slate-300 uppercase tracking-tight group-hover:text-blue-600 transition-colors">Public</span>
                                    <input 
                                        type="checkbox" 
                                ...
                                <span className="text-[8px] text-slate-400 font-bold uppercase tracking-wide">Visible to everyone</span>
                            </label>
                        </div>

                        {submitError && (
                            <p className="text-[10px] text-rose-500 font-bold uppercase tracking-wider text-center pt-2">{submitError}</p>
                        )}
                    </div>
                    <div className="flex gap-2.5 mt-5">
                        <button onClick={onClose} className="flex-1 py-2 text-xs rounded-xl bg-slate-100 dark:bg-white/5 font-black text-slate-500 dark:text-slate-300 hover:bg-slate-200 transition-colors uppercase tracking-wider">Cancel</button>
                        <button
                            disabled={loading}
