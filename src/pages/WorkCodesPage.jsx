...
export default function WorkCodesPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const resp = await workCodesApi.getAll();
      const allData = Array.isArray(resp) ? resp : (resp?.items || []);
      setData(allData);
      setTotalCount(allData.length);
    } catch (err) {
      toast("Failed to load work done codes", "error");
    } finally {
      setLoading(false);
    }
  }, [toast]);
  ...
  const handleModalSubmit = async (payload) => {
    try {
      if (editItem) {
        await workCodesApi.update(editItem.id, payload);
        toast("Work code updated");
      } else {
        await workCodesApi.create(payload);
        toast("Work code created");
      }
      setModalOpen(false);
      setEditItem(null);
      fetchData();
              ...
              <table className="w-full text-left border-separate border-spacing-y-1 min-w-max text-[11px] text-black">
                <thead className="sticky top-0 z-10 text-black">
                  <tr className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 h-[56px] text-black">
                    <th className="px-5 pl-8 h-[56px] text-[10px] font-black uppercase tracking-widest text-black text-left">Code</th>
                    <th className="px-5 h-[56px] text-[10px] font-black uppercase tracking-widest text-black text-center">Description</th>
                        ...
                        <td className="px-5 rounded-r-2xl h-[60px] text-center transition-colors text-black">
                          <ActionsMenu
                            ...
                            onAuditLog={canViewAuditLog ? () =>
                              navigate(`/audit-logs?primaryKey=${row.id}&entityName=WorkDoneCode`)
                              : undefined
                            }
                          />
                        </td>
